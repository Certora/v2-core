...
contract PriceOracle is IPriceOracle, BaseImplementation {
    ...
    uint256 public constant FP_DECIMALS = 18;

    // Maximum number of decimals supported for a token when computing inverse quotes: 36
    uint256 public constant INVERSE_FEED_MAX_DECIMALS = 36;
     ...
     * @return decimals Decimals of the requested price rate
     */
    function _getInversePrice(address inverseFeed) public view returns (uint256 price, uint256 decimals) {
     ...
     */
    function _scalePrice(uint256 price, uint256 priceDecimals, uint256 resultDecimals) public pure returns (uint256) {
        return
