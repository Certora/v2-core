...
// Declaration of contracts used in the spec
 ...
 using PriceOracleHarnessNoMunging as oracle;
...
methods {
    ...
    //Strategy functions:
    function _.claim(bytes) external => DISPATCHER(true);
    function _.join(address[], uint256[], uint256, bytes) external => DISPATCHER(true);
    function _.exit(address[], uint256[], uint256, bytes) external => DISPATCHER(true);
    function _.lastValue(address) external => DISPATCHER(true);

    //packages/bridge-connector/contracts/BridgeConnector.sol:
    function _.bridge(uint8, uint256, address, uint256, uint256, bytes) external => DISPATCHER(true);
...
definition delegateCalls(method f) returns bool = 
    ...
    f.selector == sig:swap(uint8,address,address,uint256,SmartVaultHarnessNoMunging.SwapLimit,uint256,bytes).selector);
...
filtered {f -> f.selector == sig:claim(address,bytes).selector || 
              f.selector == sig:bridge(uint8,uint256,address,uint256,SmartVaultHarnessNoMunging.BridgeLimit,uint256,bytes).selector || 
              f.selector == sig:join(address,address[],uint256[],uint256,bytes).selector ||
              f.selector == sig:exit(address,address[],uint256[],uint256,bytes).selector ||
              f.selector == sig:swap(uint8,address,address,uint256,SmartVaultHarnessNoMunging.SwapLimit,uint256,bytes).selector ||
